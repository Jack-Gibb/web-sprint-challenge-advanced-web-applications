...
export default function App() {
  ...
  const getArticles = () => {
    ...
    axios.get(articlesUrl, { headers: { Authorization: token } })
      .then(res => {
        setArticles(res.data.articles) // Ensure articles data structure is correct
        setMessage(res.data.message)
      })
      .catch(err => {
  ...
  const postArticle = article => {
    ...
    axios.post(articlesUrl, article, { headers: { Authorization: token } })
      .then(res => {
        setArticles([...articles, res.data.article]) // Ensure articles data structure is correct
        setMessage(res.data.message)
  ...
  const updateArticle = ({ article_id, article }) => {
    ...
    axios.put(`${articlesUrl}/${article_id}`, article, { headers: { Authorization: token } })
      .then(res => {
        setArticles(articles.map(a => a.article_id === article_id ? res.data.article : a)) // Ensure articles data structure is correct
        setMessage(res.data.message)
        setCurrentArticleId(null)
      })
      .catch(err => {
